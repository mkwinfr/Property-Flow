...
export function OperationsOverview({ onNavigate }: { onNavigate: (destination: string) => void }) {
  ...
  const portalRequests = workOrders.data?.workOrders.filter((item) => item.submissionSource === "portal" && !["complete", "cancelled"].includes(item.status)) ?? [];
  const metrics = [
    { label: "Open work orders", value: data.workOrders.open, detail: `${data.workOrders.emergency} emergency · ${data.workOrders.overdue} overdue`, icon: Wrench, tone: "green", to: "/work-orders" },
    { label: "Draft inspections", value: data.inspections.draft, detail: `${data.inspections.damageFound} inspections with damage`, icon: ClipboardPen, tone: "amber", to: "/inspections?context=maintenance" },
    { label: "Low stock", value: data.inventory.lowStock, detail: `$${data.inventory.totalValue.toLocaleString(undefined, { maximumFractionDigits: 0 })} inventory value`, icon: PackageSearch, tone: "coral", to: "/inventory" },
    { label: "Pool compliance", value: data.pool.exceptions, detail: data.pool.latestLogDate ? `Last logged ${formatDate(data.pool.latestLogDate)}` : "No readings logged", icon: Waves, tone: "blue", to: "/pool-logs" },
  ];
  return <>
    {portalRequests.length > 0 && <section className="panel portal-work-order-banner"><div className="panel__heading"><div><p className="eyebrow">Resident portal</p><h2>{portalRequests.length} open portal request{portalRequests.length === 1 ? "" : "s"}</h2></div><button className="button button--primary" onClick={() => onNavigate("/work-orders")}>Review queue <ArrowRight size={16} /></button></div><div className="ops-list">{portalRequests.slice(0, 3).map((item) => <div className="ops-row" key={item.id}><span className="ops-priority ops-priority--green"><Wrench size={15} /></span><span><strong>{item.title}</strong><small>Unit {item.unitNumber} · {item.residentName ?? item.requestedBy ?? "Resident"}</small></span><span className="work-order-portal-badge">Portal</span><span className={`work-status work-status--${item.status}`}>{item.status.replaceAll("_", " ")}</span></div>)}</div></section>}
    <section className="metric-grid">{metrics.map(({ icon: Icon, to, ...metric }) => <button className={`metric-card metric-card--${metric.tone} metric-card--button`} key={metric.label} onClick={() => onNavigate(to)}><span className="metric-card__icon"><Icon size={20} /></span><div><small>{metric.label}</small><strong>{metric.value}</strong><p>{metric.detail}</p></div></button>)}</section>
