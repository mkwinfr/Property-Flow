...
const STAFF_PATHS = [
  "/", "/my-work", "/turns", "/units", "/pool-logs", "/operations", "/templates",
  "/administration", "/audit", "/residents", "/leasing", "/communications", "/financial", "/platform-admin",
];
...
function StaffApp({ path, navigate }: { path: string; navigate: (to: string, options?: { replace?: boolean }) => void }) {
  ...
  const validPath = STAFF_PATHS.includes(path) || /^\/my-work\/[^/]+$/.test(path) || /^\/turns\/[^/]+$/.test(path);
  useEffect(() => {
